...
class Protocol
{
	...
	public:
		/*
		@desc. Provides read and write access to an nxn array of booleans stored in EEPROM. See init(char dim, int startAddr) for how Protocol instances will differ. The first byte stores the dimesnion n of the array. Subsequent bits store booleans, row by row.
		@future improvements
			-Allow 
				rectangular arrays.
		...
		*/
		...
		/*
		@desc. Initializes with parameters.
		@in
		...
		*/
		...
		/*
		@desc Flushes any outstanding data to EEPROM, since there are no guarantees on when data is written. An init(...) which returns true must be called before flush(). This must be called when all writes are finished to guarantee everything is pushed to EEPROM.
		@out Whether the flush succeeded.
		*/
		boolean flush();
		
		/*
		@desc Dumps data to the serial port in a comma-separated list of bytes. An init(...) which returns true must be called before dump().
		@out Whether the dump succeeded.
		*/
		boolean dump();
